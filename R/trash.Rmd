...
##get citation location (block)
```{r citation_block}
  block <- trash_citations$block
  trash_citations$block <- as.factor(trash_citations$block)
  
  ggplot(data = trash_citations) +
    geom_bar(mapping = aes(x = block)) + coord_flip()+
    theme(axis.text.y = element_text(size=5)) 
  
```
```{r citation type}
  ggplot(data = trash_citations) +
    geom_bar(mapping = aes(x = block, fill=description)) + coord_flip()+
    theme(axis.text.y = element_text(size=5))
```
