...
```{r get_citations, code=xfun::read_utf8('get_citations_script.R')}
...
```{r get_blocks, code=xfun::read_utf8('get_area_blocks_script.R')}
...
The location and the type of 311 service requests in the **`r params$area_name` `r params$area_type`**
...
area_requests %>% 
  ...
  theme_void() +
  labs(
    title = glue::glue("Service Requests by type in {knitr::combine_words(params$area_name)}"),
...
area_citations %>% 
  ...
  labs(
    color = "Description",
    title = glue::glue("ECB Citations in {knitr::combine_words(params$area_name)}"),
...
street_blocks %>% 
  ...
  theme_void() +
  labs(
    title = glue::glue("Numbered blocks in {knitr::combine_words(params$area_name)}")
  )
...
area_citations %>%
  ...
  ylab("Total Citations") +
  labs(
    title = glue::glue("Citations Over Time in {knitr::combine_words(params$area_name)}"),
...
block_street_count %>%
  ...
  gt() %>%
  tab_header(
    title = glue::glue("10 blocks in {knitr::combine_words(params$area_name)} with the least citations since {params$year_start}")
  ) %>%
  cols_label(
    block_number = "Block",
    street_name = "Street",
...
area_citations %>% 
  ...
  theme_void() +
  labs(
    title = glue::glue("Unpaid Citations in {knitr::combine_words(params$area_name)}"),
    caption = citations_source,
